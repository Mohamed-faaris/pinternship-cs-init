...
• 
When Alice sends $100 to Bob, Alice’s balance must decrease and Bob’s must
increase—both or neither.
• 
If a payment and a transaction log update don’t both succeed, the system risks
lost money or double credits.
• 
Regulatory compliance (banking, fintech) demands that transaction records are
always correct—even if the server crashes or the network fails.

![Sample Image](https://i.postimg.cc/5tC8HFJk/react3.png)

The challenge:How can FinTrust guarantee that every transfer, payment, or refund is always
complete, correct, and never leaves the system in a broken state, even when
updating multiple documents or collections?
...
• 
Know how to use MongoDB’s multi-document transactions for atomic, consistent,
isolated, and durable operations.
• 
Learn the syntax and workflow for starting, committing, and aborting
transactions.
• 
...
• 
Every step is logged, and if the power fails mid-transfer, the system
automatically reverses everything.

MongoDB’s multi-document transactions are like this double-lock vault system:They guarantee that all related changes succeed or none do, keeping everyone’s
money safe.
...
• 
Atomicity: All operations in a transaction succeed or none do.If Alice’s balance decreases, Bob’s must increase, or both are rolled back.
• 
Consistency: Transactions always leave the database in a valid state.No user can have a negative balance after a transfer.
• 
Isolation: Transactions don’t interfere; each sees a consistent snapshot.No one sees Bob’s balance increase until Alice’s decrease is committed.
• 
Durability: Once committed, the changes survive crashes or power failures.

Why is this important?Without ACID, financial systems can lose money, double-spend, or create audit
nightmares—unacceptable in banking, fintech, and regulated industries.
...
• Transactions add overhead; use only when needed for business logic and
...
```
{
  ...
  "to": ObjectId("665f4d7e8b3e6c1e24a7b3e3"),
  "amount": 100.00,
...
• If any step fails (e.g., Alice doesn’t have enough balance), all changes are
rolled back—no partial transfers or missing logs.
...
Your Turn!You’re building a new feature for FinTrust:
...
Ensure:
  ...
  ◦ 
If any step fails (e.g., recipient doesn’t have enough balance to refund), no
changes are made.
  ◦ 
