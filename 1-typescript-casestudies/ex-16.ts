// Source: https://sudarshansudarshan.github.io/pinternship/case-studies/16-design-patterns/

/*
Problem Statement:

CoffeeShip Café is growing and needs repeatable, maintainable solutions for coordination and extensibility (e.g., only one manager, flexible object creation, notifying many subsystems, swapping algorithms).

The challenge: How do you implement and apply common design patterns (Singleton, Factory, Observer, Strategy) in TypeScript to make the system modular, testable, and easy to extend?
...
• Use Singleton for single-instance needs (config, logger).
• Use Factory for flexible object creation.
• Use Observer for event notifications.
• Use Strategy to swap behaviors without changing callers.
*/
...
• 
When a drink is prepared, inventory, baristas, and the customer all need
updates-if one is missed, chaos erupts.
• 
The café wants to offer seasonal promotions and different ways to prepare
drinks, but changing the workflow is risky.

![Coffee shop challenge](https://i.postimg.cc/vTPDMM6J/day-4-tut2.png)

How can CoffeeShip Café keep operations smooth, reliable, and ready to grow-even
as new drinks, staff, and features are added?
...
• 
Recognize and implement Singleton, Factory, Observer, and Strategy design
...
• 
Factory: The kitchen has a recipe book for making any drink, so new drinks can be added
easily.
• 
Observer: When a drink is ready, all relevant staff (inventory, barista, customer) are
instantly notified-no one is left out.
• 
Strategy: The barista can use different preparation styles (fast, eco, strong) for each
order, and can swap styles as needed.

# Design Patterns

## What Are Design Patterns?

Design patterns are repeatable solutions to common problems that come up when designing software systems. Think of them as “best practice
templates” or “recipes” that experienced developers use to solve certain types of
challenges.They aren’t finished code, but rather ways of organizing your program so it’s easier to build, change, and understand.
...
• 
What it is:Ensures there is only one instance of a particular object throughout your entire application
• 
Analogy:Like a café with only one manager-no matter how many times you ask for the
manager, you always get the same person.
• 
Why use it:To manage things that should only exist once, such as a settings manager,
database connection, or logging service. Example: ```typescript class CafeManager { private static instance: CafeManager; private
constructor() {} static getInstance(): CafeManager { if (!CafeManager.instance)
{ CafeManager.instance = new CafeManager(); } return CafeManager.instance; } }
// Usage: const manager1 = CafeManager.getInstance(); const manager2 =
CafeManager.getInstance(); console.log(manager1 === manager2); // true
...
• Key point:You can add new drinks (types) without rewriting the whole menu.
...
• 
What it is:Lets one object (the “subject”) notify many other objects (the “observers”) when something changes.
• 
Analogy:Like a barista ringing a bell when a drink is ready-everyone who cares
(inventory, customer, promotion system) hears the bell and reacts.
• 
Why use it:To keep different parts of your system in sync without tightly coupling them.
• Example: 
```
...
```
• Key point:When something important happens, everyone who needs to know is notified
automatically.
...
• 
What it is:Lets you swap out different ways of doing something (an “algorithm”) without changing the object that uses it.
• 
Analogy:Like a barista choosing how to prepare a drink: fast, eco-friendly, or extra
strong-just swap the recipe card.
• 
Why use it:To change behavior on the fly, or to keep your code flexible and clean.
• Example: 
```
...
```
• Key point:You can switch how things are done without rewriting the barista.
...
Master Dependency Injection-the secret ingredient for building flexible, testable, and loosely coupled
systems.Imagine your café’s staff and machines always getting the right tools,
ingredients, and helpers-automatically!
