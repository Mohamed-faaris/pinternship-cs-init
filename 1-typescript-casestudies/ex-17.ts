...
EduFlow Academy is a rapidly growing EdTech Startup.But chaos is brewing:
...
• Admins want to review everything, but sometimes they accidentally change published or
sensitive content.
• ![Role-Based Access](https://i.postimg.cc/W3gcGZVP/image-1.png)

Your mission:Design a system so that:
...
Expected outcome:A secure, organized platform where each role has just the right level of access,
and shared features are coded only once.
...
Think of your code as an office building: | Area/Room | Access Modifier | Who Can Enter? | What Happens Here? |
|———————-|——————|————————————————–|——————————————| | Lobby & Hallways | public | Everyone
(employees, managers, directors) | Viewing shared info, casual meetings | |
Manager’s Office | private | Only the manager (the class itself) | Working on
sensitive projects | | Shared Meeting Room | protected | Managers and directors
(class & subclasses) | Team planning, confidential reviews | | Building Blueprint |
Abstract Class | Blueprint for all rooms | Sets required features for all rooms |
...
abstract class: The blueprint for all rooms; you can’t walk into a blueprint, but every room
follows its design.
...
A class in TypeScript is a blueprint for creating objects with specific properties
(fields) and methods (functions).Classes support object-oriented programming concepts like encapsulation, inheritance, and abstraction
...
• Not an access modifier, but ensures the value can’t be changed after
initialization. 
```
class Book {
...
TypeScript allows you to declare and initialize properties directly in the
constructor using access modifiers
...
Unlock the power of Generics!Imagine designing tools (like boxes or folders) that magically adapt to hold any
kind of content-quiz, lesson, or assignment. You’ll learn how to build these
flexible, reusable tools in TypeScript!
