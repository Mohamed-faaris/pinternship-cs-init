...
Analogy: The School Principal’s Decision BookJust as a principal uses a well-organized “Decision Book” of rules to manage students, your TypeScript code uses conditional statements to make decisions in a clear, reliable way. Let’s unpack each rule in the book and see how it maps to TypeScript constructs:
...
1. `checkSign(num: number): void`Use an `if` statement to log whether `num` is positive.
2. `evenOrOdd(num: number): void`Use an `if…else` to log whether `num` is even or odd.
3. `getGrade(score: number): string`Use an `if…else if…else` ladder to return a letter grade:
  ...
  ◦ `score ≥60 → "D"`
  ◦ otherwise `"F"`
4. `provideFeedback(grade: string): void`Use a `switch` to log a feedback message for each grade (`"A"… "F"`), with a `default` for any unexpected value.
