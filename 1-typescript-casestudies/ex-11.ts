...
1. Inspecting Each Bin (Loops)The belt never stops unless you tell it to. You pick up each bin one by one, inspect its label, and decide what to do. This is your loop-processing each transaction in order.
2. Skipping Cancellations (`continue`)Some bins are marked “Cancelled”-no action needed. You glance at them, set them aside, and wave the belt on. In code, you use `continue` to skip the rest of the steps for that iteration and move to the next bin.
3. ...
...
# Mastering Loops in TypeScript
...
## 3. Concept Introduction with Analogy
...
Handling Priority Requests (`break`)Every so often, a bin arrives with a “PRIORITY” sticker-this is an urgent reservation. You press the big red button and stop the belt so you can process that first. In code, you use `break` to exit the loop immediately and deal with high-priority work.
4. Ensuring at Least One Return (`do…while`)Even if the day’s returns aren’t queued, policy says you must check for a return at least once (in case a librarian added one while you were busy). You reach out, inspect a bin, and then keep checking until you’ve processed every return. That first guaranteed check is like a `do…while`-run at least once, then repeat if the condition holds.
5. Keeping Daily Tallies (Loop Counters)At a corner of your workspace is a clipboard. Every time you handle a checkout, ...
...
# Mastering Loops in TypeScript

## 3. Concept Introduction with Analogy
...
return, or priority, you tick a box on that clipboard. Your loop counters in code do the same-incrementing counts for each transaction type so you can report totals at day’s end.
6. Reviewing the Inventory Ledger (`for…in`)Between belt cycles, you glance at the library’s Inventory Ledger, a book listing each title and its current stock. You flip through each entry-this is akin to a `for…in` loop over an object’s keys, checking “The Hobbit,” then “1984,” then “TypeScript Guide,” and so on.
7. Greeting Visitors (`for…of`)At the end of your shift, you have a list of visitors waiting at the front desk. You call out each name and welcome them: “Alice, your book is ready. Bob, enjoy your reading.” That’s a `for…of` loop-iterating directly over an array of visitor names and performing an action ...
...
# Mastering Loops in TypeScript
...
- [←](https://sudarshansudarshan.github.io/pinternship/case-studies/10-conditional-logic-in-typescript/)
