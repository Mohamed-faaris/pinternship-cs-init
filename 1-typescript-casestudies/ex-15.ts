...
![Image](https://i.postimg.cc/mr8yGgjc/vibe-fin.png)
...
• 
Transform or filter data for any report, without rewriting code for every new
dashboard feature?
...
• 
Sometimes you want to quickly reshape a tool for a special job (transform
types).
• 
Sometimes you want a tool that adapts itself based on the job (conditional
types).

Advanced types let you build tools that are just as flexible and adaptable as your real-world
needs.
...
A union type allows a value to be one of several types.It’s written with the `|` (“or”) operator.

## How They Work

• 
When a value is a union, TypeScript only allows you to use members/methods that
...
• 
If there’s a property with the same name but different types, TypeScript will
error.
...
```typescript const alice: MultiRoleUser = { id: “alice123”, quizzesCompleted:
10, coursesTaught: 2 };
...
• 
Mapped Types: When you want to transform or generate types based on existing ones (e.g., make
all fields optional).
• 
Conditional Types: For type-level logic and type transformations (e.g., extracting types,
enforcing rules).
• 
...
Congratulations!You’ve mastered the Swiss Army knife of TypeScript types.Next, imagine combining all these tools to build a reporting engine that adapts
to any new content or user type ViBe invents in the future!
