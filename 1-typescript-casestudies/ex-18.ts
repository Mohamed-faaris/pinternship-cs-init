...
The problem:How do you provide the payment module with the right gateway, swap gateways
easily, and test without real payments while keeping your payment logic decoupled and flexible?
...
• 
The terminal doesn’t care what kind of plug (gateway) is used; it just needs a
compatible socket.
• 
You can plug in a Stripe adapter, a PayPal adapter, or a test adapter for
training.
• 
If you upgrade to a faster plug, you don’t need to change the terminal-just swap
the plug.

Dependency Injection is like using a universal socket:You can swap adapters (dependencies) without changing the device (business
logic).

## What is Dependency Injection?

• 
Dependency Injection (DI) is a design pattern where an object’s dependencies are provided (“injected”)
from outside, rather than hardcoded inside the object.
• 
Inversion of Control (IoC): The control of creating and supplying dependencies is inverted-handled by an
external entity, not the object itself.
...
For testing, inject a mock gateway that simulates payment without real
transactions.
...
You’ve mastered Dependency Injection basics!Next, explore IoC Containers (like InversifyJS) for automatic dependency management and advanced DI
features.
