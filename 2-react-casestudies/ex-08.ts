...
• 
Some components (like charts and video overlays) are expensive to render or
compute.
• 
User interactions (like filtering comments or toggling overlays) can cause
unnecessary re-renders, slowing down the UI.
• 
The team wants to optimize performance, especially as the dashboard grows more
complex and data-intensive.

![Sample Image](https://i.postimg.cc/kGzjSRG1/react11.png)

The challenge:How do you prevent unnecessary recalculations and re-renders in React, ensuring
the UI remains fast—even as state and props change frequently?
...
• 
useMemo is like a results whiteboard: If you’ve already done a complex calculation
(like analyzing a video feed), you write the result on the board and reuse it until
the inputs change.
• 
useCallback is like a phone directory: You keep the same phone number (function reference)
for callbacks, so you don’t have to reprint the directory every time someone’s
name changes.
• 
React.memo is like a smart camera operator: They only update the camera angle (re-render)
if the scene actually changes, not just because someone walked into the control
room.
...
• 
Memoization is the process of caching the result of a function so it doesn’t need to be
recomputed unless its inputs change.
• 
In React, memoization prevents unnecessary recalculations and re-renders,
improving performance—especially in large or complex UIs.
...
• 
When you have a computation that is expensive (heavy calculation, large
filtering, etc.).
• 
When the computed value is used in rendering and only depends on specific props
or state.
...
• 
If dependencies are unstable (e.g., new object/array each render), memoization
is ineffective.
...
• 
Overusing `useCallback` can add complexity with little benefit if the function is cheap or the child
isn’t memoized.
• 
Be careful with dependencies—stale closures can occur if dependencies are
missing.
...
• 
`React.memo` wraps a component and only re-renders it if its props change (shallow
comparison).
• 
...
1. 
Create a `TagList` component that:
...
3. 
Show how changing unrelated state in the parent does not re-render the memoized `TagList` or `TagInput`.
