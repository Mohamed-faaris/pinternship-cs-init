...
• 
Each feature (files, users, comments, notifications, preferences) is managed by
a different team.
• 
...
Performance is critical—only components using changed state should re-render.
• 
Teams want to use middleware (devtools, persistence, logging) on specific state
slices.

![Sample Image](https://i.postimg.cc/0jRMJ8xG/react9.png)

The challenge:How do you architect a global state system that is modular, type-safe, and scalable—so each feature team can own their slice, and the app remains fast and
maintainable?
...
• 
Slices are like specialized teams in a control tower: one team manages flights, one
manages weather, one manages communications.
• 
Each team (slice) has its own dashboard, rules, and logs—but they all work
together in the same tower (store).
• 
If the weather team updates a forecast, only the weather dashboard changes—not
the flight or comms dashboards.
...
• 
A slice is a function that returns a piece of state and its actions, with its own
types.
• 
...
test(‘setUser sets user’, () => { let state = { user: null }; const set = (fn)
=> { state = { …state, …fn(state) }; }; const slice = createUserSlice(set);
slice.setUser({ id: ‘u2’, name: ‘Sam’ }); expect(state.user.name).toBe(‘Sam’); });
...
3. 
Build a `NotificationsPanel` component that displays unread notifications and lets users mark them as read.
