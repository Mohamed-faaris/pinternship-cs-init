...
• 
It offers courses, quizzes, video lectures, forums, and a dashboard—all as
separate features.
• 
Most users only use a few features per session (e.g., watching a video or taking
a quiz).
• 
Loading the entire app upfront slows down the initial experience, especially on
slow networks.
• 
The team wants to load only the code needed for the current page or feature, and
fetch the rest on demand.

![Sample Image](https://i.postimg.cc/2y6Jhfw6/Chat-GPT-Image-May-31-2025-12-43-02-AM.png)

The challenge:How do you split your React app into smaller bundles and load them only when
needed—improving initial load times, reducing bandwidth, and keeping the user
experience smooth?
...
• 
Understand what code splitting and lazy loading are, and why they matter for
React apps.
• 
...
• 
Apply both route-based and component-based code splitting for maximum
performance.
• 
Understand best practices, pitfalls, and how to handle errors and loading
states.
...
• 
Instead of opening all classrooms at once (loading all code), you unlock each
classroom only when a student needs it (lazy loading).
• 
The janitor (React) keeps the main hall (core app) open, but unlocks classrooms
(feature bundles) on demand, saving energy and time.
...
• 
Code splitting breaks your app into smaller chunks (bundles) so users only download what they
need.
• 
Lazy loading means loading code (components, modules) only when it’s actually required, not
upfront.
• 
...
• 
Can be used conditionally, in event handlers, or anywhere in your code—not just
at the top. ```js


// Only load the math module if needed function handleCalculate() {
...
• 
Each route loads its own bundle. ```js import { BrowserRouter as Router, Routes,
Route } from ‘react-router-dom’; import { Suspense, lazy } from ‘react’;


const Dashboard = lazy(() => import(‘./Dashboard’)); const Courses = lazy(() =>
import(‘./Courses’)); const Forum = lazy(() => import(‘./Forum’));


function App() { return ( <Suspense fallback={<div>Loading page...</div>}> <Route path="/" element={} /> <Route path="/courses" element={} /> <Route path="/forum" element={} /> </Suspense> ); }
...
• 
Lazy loading too many small components can increase network requests and
overhead.
• 
...
• 
Dynamic import paths must be static strings (not variables) for bundlers to
split correctly.
...
• Useful for loading third-party libraries, modals, or admin tools only when
needed.
...
1. 
Create a `ProfileSettings` component that is only loaded when the user clicks a “Settings” button.
2. 
Use `React.lazy()` and `Suspense` to load the component with a loading spinner.
3. 
Add a route `/admin` that lazy-loads an `AdminPanel` component only when visited.
4. 
Show how to handle loading errors with an error boundary.
...
• 
Start with route-based code splitting, then optimize with component-based
splitting.
• 
