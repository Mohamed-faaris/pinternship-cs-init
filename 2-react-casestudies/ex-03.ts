...
• 
The dashboard must display details for the correct patient/appointment and
prevent type errors (e.g., using a string where a number is expected).
• 
Developers want to catch mistakes at compile-time, not after deployment (e.g., a
route handler expecting a `number` but getting `undefined`).
• 
The codebase must remain maintainable as routes grow more complex.

![Sample Image](https://i.postimg.cc/QVG237XG/ract5.png)

The challenge:How do you define, use, and enforce type-safe dynamic route parameters in React
Router, so your navigation and data fetching are both robust and error-free?
...
• 
Routes are like appointment slips: Each slip has fields (patient ID, appointment ID)
that must be filled out correctly.
• 
TypeScript is the receptionist: It checks every slip for missing or mistyped info before
sending it to the doctor.
• 
React Router is the hospital’s navigation system: It makes sure each
patient/doctor/appointment page receives the right information, in the right format, every time.
...
• 
In TypeScript, you can specify the expected param types using a generic ```js
import { useParams } from ‘react-router-dom’;


interface AppointmentParams { patientId: string; appointmentId: string; }


const AppointmentDetails: React.FC = () => { const { patientId, appointmentId }
= useParams(); // patientId and appointmentId are now strongly typed as string return (
<div> <h1>Patient: {patientId}</h1> <h2>Appointment: {appointmentId}</h2> </div> );
};
...
• Use the `Link` or `useNavigate` to create URLs with params: ```js import { Link } from ‘react-router-dom’;
...
• 
Libraries like [react-router-typesafe-routes] and [react-router-typed-object]
provide helpers for even stricter typing, including:
...
• 
You can also infer types from route path strings using TypeScript conditional
types and template literals.
...
4. 
Add a link from a doctor list to a specific doctor/patient page, passing the IDs
as parameters.
