...
• 
Rules like "members can’t borrow more than 5 books" are buried in code that
handles website requests. When policies change, updates risk breaking other parts of
the system.
• 
The team wants to add new features (e.g., online reservations, late fines) but
struggles because data access and business logic are tangled together.
• 
Testing is nearly impossible-every small change requires rewriting large parts
of the code.

![Image](https://i.postimg.cc/X7YJvTf5/Chat-GPT-Image-May-19-2025-04-38-15-PM.png)

The challenge:How do you redesign the system so that:

• 
Rules and policies are clearly separated and easy to update?
• 
Data storage details don’t interfere with how books are managed or loans are
processed?
• 
...
1. 
Front Desk (Controller): Handles member requests (checkouts, returns).
2. 
Policy Team (Service): Manages rules (loan limits, fines).
3. 
Archives Team (Repository): Stores and retrieves books from the shelves.
...
• 
If the archives team reorganizes the shelves (changes storage systems), the
other teams keep working as usual.
...
• 
Dependency Injection (DI): Provide components with their dependencies (e.g., services, repositories)
instead of letting them create their own.
• 
Benefits:
...
![Image](https://i.postimg.cc/bvSpTRHw/day10.png)
...
| Pitfall | Best Practice | |——————————————|———————————————————-| | Mixing data access in business logic
| Always use repositories for storage access | | Hardcoding storage details |
Depend on interfaces, not implementations | | Not testing with mocks | Use fake
...
• 
Review regularly: Refactor if business logic creeps into controllers or data access leaks into
services.
