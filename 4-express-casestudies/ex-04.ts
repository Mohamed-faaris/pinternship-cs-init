...
FreshMart, a bustling grocery chain, launched a digital loyalty program to
reward customers. But chaos ensued:

• 
Missing Data: Customers tried to redeem points, but the app crashed if their profile was
incomplete.
• 
Invalid Requests: A customer entered "1000 points" instead of "100," redeeming $1,000 by
accident.
• 
Confusing Responses: Staff couldn’t tell if a request failed due to a server error or invalid input.

The breaking point:A customer named Sarah tried to redeem points for a birthday cake. The app
accepted her request but didn’t check if she had enough points. The system deducted
500 points (her entire balance), but the cake order failed because the store was
out of stock. Sarah left furious, and FreshMart’s reputation tanked.

Your mission:Redesign FreshMart’s loyalty API to:
...
• 
If a waiter (request) doesn’t specify "no gluten" (missing data), the kitchen
(API) might serve a dish that makes the customer sick.
• 
If the chef (API) doesn’t check stock (validation), they might promise a cake
they can’t deliver.
• 
Quality control (TypeScript types) ensures every order (request) has the right
details before the kitchen starts cooking.

A. Typing Requests and Responses

1. Defining Strict Interfaces

Problem: Without types, requests like `{ points: "100" }` (string) might be processed as `100` (number), causing bugs.Solution: Use TypeScript interfaces to enforce data shapes.
...
B. Validating Data with Zod 1. Schema Validation

Problem: TypeScript types are stripped at runtime. Malicious clients can still send
invalid data.Solution: Use Zod to validate requests at runtime.
...
1. 
Add a `POST /transfer` endpoint allowing customers to transfer points to another account.
2. 
Validate:
