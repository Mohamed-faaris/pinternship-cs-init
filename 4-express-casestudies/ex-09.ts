...
• 
Some departments use spreadsheets, others use databases, and a few still keep
paper records.
• 
When a student tries to enroll, their request is sometimes lost, or two systems
accidentally double-book a seat.
• 
If the university wants to upgrade from spreadsheets to a new database, they
worry about rewriting all their registration logic.
• 
Testing new features is risky, because the code is tightly coupled to the
current storage method.

![Image](https://i.postimg.cc/4yrPnWRr/Riverdale.png)

The challenge:How can you build a course registration system where the rules and features work
no matter how or where the data is stored-so you can upgrade, test, and scale
...
• 
The office staff don’t care if records are in filing cabinets, spreadsheets, or
a fancy database-they just use a standard form.
• 
If the university upgrades to a new system, the process for students and staff
stays the same.
...
• 
The rest of the university never worries about how records are stored or
retrieved.
...
The Repository Pattern is a design pattern that centralizes data access logic in
a single place, separating it from business logic.

• 
Purpose:

  ◦ 
Abstracts the details of data storage and retrieval from the rest of the
application.
  ◦ 
Provides a collection-like interface for accessing domain objects.
  ◦ 
Makes it easy to swap storage backends (e.g., memory, file, database) without
...
Business Logic Layer (Service):
  ...
  ◦ Uses the repository interface, not the implementation, to enforce rules and
policies.
...
• 
Never expose storage-specific types or queries (e.g., SQL, ORM objects) to
business logic.
• 
...
```typescript // repositories/InMemoryCourseRepository.ts import {
ICourseRepository } from './interfaces/ICourseRepository'; import { Course } from
'../models/Course';


export class InMemoryCourseRepository implements ICourseRepository { private
courses: Course[] = [];
...
async findById(id: string): Promise<Course | null> { return
this.courses.find(course => course.id === id) || null; }


async save(course: Course): Promise { const idx = this.courses.findIndex(c => c.id === course.id); if (idx >= 0) {
this.courses[idx] = course; } else { this.courses.push(course); } }


async enrollStudent(courseId: string, studentId: string): Promise { const course = await this.findById(courseId); if (course &&
!course.students.includes(studentId)) { course.students.push(studentId); await this.save(course);
} }

async findByStudentId(studentId: string): Promise<Course[]> { return
this.courses.filter(course => course.students.includes(studentId)); } }
...
```typescript // services/CourseService.ts import { ICourseRepository } from '../repositories/interfaces/ICourseRepository';

export class CourseService { constructor(private courseRepo: ICourseRepository)
{}

async enroll(courseId: string, studentId: string) { const course = await
this.courseRepo.findById(courseId); if (!course) throw new Error('Course not found');
if (course.students.length >= course.capacity) throw new Error('Course full');
await this.courseRepo.enrollStudent(courseId, studentId); return { message:
'Enrolled successfully' }; }

async getStudentCourses(studentId: string) { return
this.courseRepo.findByStudentId(studentId); } }
...
Learn how to use Dependency Injection to provide repositories to your services
and controllers automatically-making your app even more modular, testable, and
ready for growth!
