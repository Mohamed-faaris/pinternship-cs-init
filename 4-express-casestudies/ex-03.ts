...
Neighborhood Food Store Network needs clear, RESTful endpoints for products.

• Requests must return clear results and proper status codes.
• Use correct methods for CRUD and proper validation.

The challenge: Implement GET/POST/PUT/PATCH/DELETE endpoints with appropriate status codes and validation in Express.


Challenge (Interactive - "Your Turn"):

1. Add a `PATCH /products/:id/inStock` endpoint to update only `inStock` status and return `400` if the new status is missing or not boolean.
*/
...
You manage a group of neighborhood food stores. Every day, staff and customers
need to:
...
If requests get mixed up or answers are unclear, staff and customers get
confused, orders are delayed, and the stores lose business. ![Image](https://i.postimg.cc/hPC3vbJ5/https-ps.png)

### The challenge:

How do you organize and handle these different requests so that everyone always
gets the right answer, and nothing falls through the cracks?

## 2. Learning Objectives

By the end of this lesson, you will be able to:

• Organize and implement all types of requests (looking up, adding, changing,
removing) in Express.
...
A well-trained waiter keeps the dining experience smooth and stress-free, just
as a well-designed food store system keeps requests and answers organized.

### A. What Is a Request and a Response?

• Request: When someone asks your food store system for information or to perform an
action.
• Response: The system’s answer to that request.

In web systems, these requests and responses travel over the internet using a
...
![Image](https://i.postimg.cc/cJDbgR5H/http4.png)
...
![Image](https://i.postimg.cc/PrhPJ1GY/https3.png)
...
• 
Never expose sensitive server errors to the client (use 500 for unexpected
issues).

## G. Visual Recap: How a Request Flows

![Image](https://i.postimg.cc/htS3PZPx/https.png)
...
|———————|——–|———————–|———————-|——————————————–| | List all products | GET | /products | 200 | Show all
products | | Get one product | GET | /products/:id | 200, 404 | Show product or not
found | | Add product | POST | /products | 201, 400 | Add new product, validate
input | | Replace product | PUT | /products/:id | 200, 400, 404 | Replace all
fields, validate input | | Update price | PATCH | /products/:id/price | 200, 400,
404 | Change price only | | Remove product | DELETE | /products/:id | 204, 404
| Remove product, no content on success |
