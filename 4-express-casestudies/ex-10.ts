...
Sunrise Family Clinic started with a single doctor and a handwritten appointment book.Now, with multiple doctors, online booking, reminders, and insurance checks:
...
![Image](https://i.postimg.cc/CMXTgJDH/Express-2.png)

The challenge:How can you design your appointment system so each part (scheduling, notifications, billing) can be developed, tested, and swapped out independently-without breaking the rest of the system?
...
• Constructor Injection (most common and recommended):Dependencies are passed as parameters to the class constructor.
...
• Property Injection:Dependencies are set directly on public properties (less common in TypeScript/Node).
...
• Service Tokens:Instead of using classes directly, you can use tokens (symbols or strings) for even looser coupling.
• Factory Functions:Register services using factories for dynamic creation or configuration.
• Scoped Containers:Create per-request or per-user containers for request-specific dependencies (e.g., user context, transactions).
• Service Groups:Inject arrays of services (e.g., multiple notification handlers).
• Resetting and Cleaning Up:Use `Container.reset()` to clear registrations between tests or for hot-reloading.
...
Clinic Scenario:The clinic knows it will need to add new notification types (SMS, Email, automated calls) and billing providers (Stripe, insurance, etc.) as it grows. They want to avoid rewriting appointment logic every time.

How DI/TypeDI Solves This:By setting up TypeDI and using interfaces, the clinic can register any notification or billing provider and swap implementations with a single line of code.
...
Clinic Scenario:The clinic wants to ensure that all notification and billing services follow a standard contract, so they can add new providers or test with mocks easily.

How DI/TypeDI Solves This:By defining interfaces, the appointment logic doesn’t care how a notification is sent or a bill is processed—only that it happens.
...
Clinic Scenario:The clinic starts with SMS for reminders, but wants to add Email later, and maybe swap in a mock for testing.

How DI/TypeDI Solves This:Each provider is a class marked with `@Service()`, making it injectable and swappable.
...
Clinic Scenario:The appointment scheduler shouldn’t care how reminders are sent or how billing is processed—it should just ask for those services.

How DI/TypeDI Solves This:The `AppointmentService` receives its dependencies via constructor injection, making it easy to swap them for different environments or tests.
...
Clinic Scenario:The clinic wants to use Email reminders for some patients, or a mock billing service for testing, without touching appointment logic.

How DI/TypeDI Solves This:You can register any implementation with the container at runtime.
...
Clinic Scenario:The clinic wants to test appointment booking without sending real notifications or charging real credit cards.

How DI/TypeDI Solves This:Inject a mock service that records calls instead of performing real actions.
