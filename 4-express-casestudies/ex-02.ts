...
Greenfield Community Center’s app grew into a tangled single-file project.

• Volunteers accidentally break unrelated features when updating code.
• The project lacks modular routes, controllers, and static assets.

The challenge: Organize an Express project into `routes/`, `controllers/`, `models/`, and `public/` so features are modular, testable, and maintainable.


Challenge (Interactive - "Your Turn"):

1. Create a `routes/contact.js` that returns contact info and mount it in `app.js`.
2. Structure the project with `routes`, `controllers`, and `public` folders.
*/
...
• 
Every new feature meant adding more code to the same file, making it harder to
read, debug, or update.
• 
When a volunteer tried to fix a bug, they accidentally broke the events page.

![Image](https://i.postimg.cc/rpQpPwQb/express3.png)

The challenge:How do you organize your Express project so it’s easy to add new features, fix
bugs, and let multiple people work together-without stepping on each other’s toes
or losing control as your app grows?
...
Imagine the community center started in one room where everything
happened-meetings, classes, lost & found, and even storage.
...
• 
Now, everyone knows where to go, and the center can offer more services without
chaos.
...
greenfield-center/ ├── app.js # Main application entry point ├── package.json #
Project metadata and dependencies ├── routes/ # Route definitions (handles
endpoint paths) │ ├── events.js │ └── classes.js ├── controllers/ # Request-handling
logic (optional for larger apps) │ ├── eventsController.js │ └──
classesController.js ├── models/ # Database models (e.g., Mongoose schemas) │ └── event.js ├──
public/ # Static files (CSS, images, client-side JavaScript) │ ├── css/ │ ├──
js/ │ └── images/ ├── views/ # HTML templates (e.g., using EJS, Pug, or
Handlebars) │ └── index.ejs ├── middleware/ # Custom middleware functions (e.g., auth,
logging) │ └── auth.js ├── .env # Environment variables (if used) └── README.md #
...
Let’s see how this structure directly solves Greenfield Community Center’s
problems:
...
```
2. 
Create `routes/events.js` as shown above.
3. 
Move your `/events` route code from `app.js` to `routes/events.js`.
...
• 
Each route is in its own file, so you can add, update, or fix features without
...
| Pitfall | Best Practice | |———————————|————————————————–| | Mixing all routes
in one file | Separate routes into their own files | | Hardcoding data
everywhere | Use controllers or models for logic | | Ignoring static files | Use public/
for images, CSS, JS | | Not using version control | Use Git to track changes and
collaborate |
