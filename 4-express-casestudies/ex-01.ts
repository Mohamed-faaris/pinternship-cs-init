...
Greenfield Community Center needs a simple, reliable web server to serve requests and events.

• The center needs routes for events, contact, and basic pages.
• Volunteers should be able to add routes without breaking the app.

The challenge: Set up an Express server with routes, static files, and a clean project structure so the center can scale features safely.


Challenge (Interactive - "Your Turn"):

1. Add a new route `/contact` that returns the center’s contact info as JSON and test it.
*/
...
• 
They want a simple website where people can check upcoming events, sign up for
classes, and send questions to the staff.
• 
Right now, all requests come in by phone or handwritten notes, which get lost or
mixed up.
• 
The center’s team is excited to put everything online, but they’ve never set up
a web server before and don’t know where to start.

![Image](https://i.postimg.cc/JzRMCWDW/express.png)

The challenge:How do you set up a simple, reliable system that listens for online requests and
responds with helpful information-so the community center can serve everyone
efficiently and never lose a request again?
...
• 
When a neighbor walks in (makes a request), Express greets them and listens to
what they need.
• 
If someone asks about upcoming events, Express checks the calendar and gives a
list.
• 
If someone wants to sign up for a class, Express writes down their info and
confirms their spot.
• 
Express never loses a note and always responds politely, making sure everyone
feels heard.
...
• 
It makes it easy to build web servers and APIs, so you can handle requests and
send responses without a lot of boilerplate code.
• 
...
• 
Node.js is a runtime that lets you run JavaScript code outside the browser, on your
computer or server.
• 
...
• 
Open your terminal (Command Prompt, PowerShell, or Terminal app). and check
installation:
...
• Create a file called `app.js` with the following code: ```typescript const express = require(‘express’);
const app = express(); const port = 3000;


// Route for the homepage app.get(‘/’, (req, res) => { res.send(‘Welcome to
Greenfield Community Center!’); });


// Start the server app.listen(port, () => { console.log(`Community Center server running at http://localhost:${port}`); });
...
• Every online visitor gets a friendly welcome message-no more lost or ignored
requests.
...
• Neighbors can now check upcoming events anytime, without calling or writing
notes.
...
• 
When a browser (or any client) connects to `localhost:3000`, Node.js hands the request to Express, which matches it to your routes and runs
the right function.
• 
...
• The community center is now “open for business” online-ready to greet every
visitor and answer every request, 24/7.
...
• 
If you see “Cannot GET /something,” make sure you’ve defined a route for that
path.

## F. Visual Flow: How Express Handles Requests

![Image](https://i.postimg.cc/gkkNCSzT/express2.png)
...
• 
This setup is the digital “front desk” for your community center-always open,
always organized.
