...
BrightFuture University’s admissions system received incomplete and invalid applications.

• Missing or malformed fields caused processing errors and unfair rejections.
• Admissions need a reliable validation layer with clear feedback.

The challenge: Use `express-validator` or `class-validator` to define validation rules and return structured errors for invalid applications.


Challenge (Interactive - "Your Turn"):

1. Add a validation rule requiring a `portfolioLink` that is a valid URL for art applicants and return a clear error if missing.
*/
...
BrightFuture University’s admissions office is flooded with thousands of online
applications every year.

• 
Some students submit forms with missing grades or essays.
• 
Others accidentally enter their birthdate as "202" or upload a selfie instead of
a transcript.
• 
Occasionally, a student’s application is processed even though their contact
info is incomplete, making it impossible to send them an offer letter.
• 
Sometimes, students are rejected for "missing documents" when they actually
uploaded everything, but the system failed to check properly.

![Image](https://i.postimg.cc/4dfPfDxh/v-ALIDATION.png)

The challenge:How do you ensure that every application is complete, accurate, and checked for
mistakes before it’s processed-so no deserving student is lost, and no time is
wasted on incomplete forms?
...
• 
Every application is checked by a staff member before it goes to the review
committee.
• 
If a transcript is missing, the staff immediately asks the student to provide
it-no application moves forward until all requirements are met.
• 
If a student writes their name in the "essay" field, the staff gently points out
the mistake and helps them fix it.
• 
Only complete, correct applications reach the decision-makers, ensuring a fair
and efficient process.

Request validation in software is like this admissions desk: it ensures that
every submission is checked for completeness and correctness before it enters the
system.
...
Let’s walk through how "BrightFuture University" ensures every application is
perfect before it’s reviewed:

## A. Designing the Application Data Structure

Picture the chaos if applications were still paper forms: some missing essays,
some with unreadable birthdates, and some with no contact info.To fix this, the university creates a clear digital structure for every
application-so nothing is forgotten, and every reviewer knows exactly what to expect.
...
• 
Every application must have a name, email, birthdate, grades, essay, and a link
to a recommendation letter.
• 
By making these fields required, the system ensures no application is missing
crucial details.
...
• 
If there are errors, the applicant gets a full list of what to fix-no more
guessing.
• 
...
• 
No more lost students: Every valid applicant gets a fair chance, and no one is rejected due to a
system oversight.
• 
...
• 
Add a validation rule that requires a "portfolioLink" field to be a valid URL
(for art applicants).
• 
If missing or invalid, return an error message: "A valid portfolio link is
...
* Learn how to combine validation with authentication and authorization, so only
